...
Reconciliation stays targeted to one semantic or provenance decision. Overlay source drift may be accepted, rejected, marked ambiguous, or identified as an absorbed delta. Source trees remain read-only; top-level aliases resolve to their canonical root, while source-internal symlinks are rejected. Local reconciliation reports the identity derived from `SKILL.md` bytes while retaining the full-directory effective checkpoint. Direct overlay payload edits route to `skill-overlay`; accepted maintenance refreshes the reviewed fingerprints atomically before preflight reruns.
