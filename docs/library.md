...
For a customization source, pass the successful nested preflight's `effectiveFingerprint` to `reconcileCustomization` as `sourceEffectiveFingerprint`; the CLI performs this nested preflight automatically. Reconciliation never substitutes the customization directory's raw fingerprint.
