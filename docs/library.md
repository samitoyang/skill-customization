...
Primary seams are descriptor validation that keeps runtime selectors inside the reviewed payload, normalization and naming, canonical-target and owned-payload fingerprints that reject internal symlinks, bounded discovery and manager records, context bindings, targeted reconciliation with entrypoint-derived local identity, recursive `preflightCustomization`, explicit `acceptMaintenanceUpdate`, and canonically contained locked atomic file/JSON updates.
