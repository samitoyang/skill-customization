...
Required top-level fields are `schema_version`, stable URI `id`, `type`, `name`, own `license`, `entrypoint`, `customization`, `dependencies`, `owned_payload`, `source`, and `activation`. Portable license, revision, review-time, and evidence strings must contain a non-whitespace character and must not encode a machine-local absolute path. `owned_payload.reviewed_fingerprint` covers every runtime-owned file except `customization.json` and reserved `provenance/` and rejects symlinks. The `entrypoint` and `customization` runtime selectors must name files inside that reviewed payload. Validation checks their canonical targets and rejects symlinks, empty or dot segments, line separators, trailing dots/spaces, alternate-data-stream separators, or filesystem aliases of either reserved path.
