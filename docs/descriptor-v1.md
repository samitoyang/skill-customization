...
Required top-level fields are `schema_version`, stable URI `id`, `type`, `name`, own `license`, `entrypoint`, `customization`, `dependencies`, `owned_payload`, `source`, and `activation`. `owned_payload.reviewed_fingerprint` covers every runtime-owned file except `customization.json` and reserved `provenance/` and rejects symlinks. The `entrypoint` and `customization` runtime selectors must name files inside that reviewed payload. Those files use canonical relative paths with no symlinks, dot segments, trailing dots/spaces, alternate-data-stream separators, or case variants of either reserved path.
