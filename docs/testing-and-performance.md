...
| Performance | `npm run test:performance` | Runs the fixture, preflight-snapshot, ambient, and emitted-artifact scenarios one at a time, each in a fresh Node.js process. Each scenario reports samples and fails its gate when robust duration or exact work budgets regress. |
...
The shared gate warms up before collecting five samples by default and reports the median, p95, maximum, and median absolute deviation. Median, p95, and scenario-specific MAD ceilings distinguish a slower operation from an unstable host; exact work budgets are host-independent and fail on unexpected ambient adapter calls, out-of-scope roots, repeated discovery work, or Git probes for candidates outside a repository. Git provenance is checked through a discovery-scoped repository marker and one cached remote lookup per repository, while each candidate's upstream entrypoint is derived locally. The artifact scenario runs the complete emitted-artifact verifier, including compilation and the emitted contract suite, with serialized emitted-test concurrency for reproducible timing, so artifact performance cannot be measured by a partial build.

## Discovery snapshot freshness

A discovery result is a request-scoped snapshot of declared roots, manager records, host configuration, plugin manifests, candidate metadata, and fingerprints at the time of the call. It is not a durable inventory cache. `preflightCustomization` creates one for the operation, or reuses the caller's already-collected inventory, and recursive binding and tracking checks select from that snapshot before using a memoized targeted lookup for a source outside the inventory.

- Reuse a snapshot only inside one operation whose discovery inputs remain unchanged; preflight owns this scope for recursive execution checks.
