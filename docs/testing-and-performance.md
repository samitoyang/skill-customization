...
| Emitted artifact | `npm run test:artifact` | Runs the emitted-artifact contract test directly. The verifier assembles one fresh isolated publication artifact and owns TypeScript compilation, declarations, source maps, emitted tests, emitted CLI/library compatibility, and the legacy CLI compatibility path. `verify:artifact` is the direct verifier and `verify:typescript` remains its alias during the migration. |
...
`npm run verify` remains the required complete correctness check and runs behavior and ambient integration before the package lane. The package lane owns the full verifier invocation, so compilation, declarations, source maps, emitted tests, CLI/library compatibility, and the package audit each run once against one artifact. `npm run test:artifact` remains available for focused artifact work without running a package audit. The performance suite is an additional check, not a replacement for correctness. CI runs both jobs. Concurrent `node:test` durations and whole-suite wall-clock time are diagnostic only; they are not performance evidence or regression gates.
