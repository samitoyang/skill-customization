...
Generated customization entrypoints are thin dispatchers. One pure canonical renderer owns their contract-1 instructions, frontmatter validation and serialization, customization-type-to-maintenance-handler mapping, and effective-workflow loading semantics. Its interface accepts only the customization type and approved skill metadata; the CLI is an adapter over the same interface. Creation workflows supply those inputs and write the returned Markdown unchanged. Repository rationale, helper fallback policy, package versions, source details, concrete paths, and context policy stay outside generated dispatchers.

Dispatchers negotiate helper contract 1 and run helper-owned preflight before any runtime instruction. A ready plan is composed before it is executed: every checked step is loaded first, the workflow is combined with semantic deltas from inner to outer, and later deltas refine earlier instructions. Only that effective workflow executes. An overlay delta is therefore part of pre-execution composition, not a post-execution hook. Stopped, unavailable, or incompatible helper states delegate to exactly one maintenance skill and end before customization instructions load.
