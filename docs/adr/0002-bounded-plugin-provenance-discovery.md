...
Cursor local plugins accept the documented root `plugin.json` and `.cursor-plugin/plugin.json` formats, including lowercase manifest identifiers, manifest-declared skill directories, per-plugin-over-marketplace precedence, and marketplace `metadata.pluginRoot` prefixes. Default `skills/` discovery is limited to child skill directories, while a root `SKILL.md` is accepted only through the documented root-skill fallback. Plugin roots may be symlinked only when their canonical paths remain inside the declared plugin root. Concrete paths and host-local plugin identities remain in discovery or binding state; portable descriptors contain stable identity and reviewed fingerprints only. The full operational contract lives in [Discovery and bindings](../discovery-and-bindings.md); `CONTEXT.md` records the vocabulary and this ADR records the rationale.
