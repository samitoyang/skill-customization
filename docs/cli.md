...
Full-directory fingerprints follow a top-level source alias to its canonical target and reject symlinks inside that target tree. Validation rejects runtime selectors under excluded `customization.json` or `provenance/` paths.
