...
Full-directory fingerprints follow a top-level source alias to its canonical target and reject symlinks inside that target tree. Validation rejects dot segments and runtime selectors under case-insensitively reserved `customization.json` or `provenance/` paths.
