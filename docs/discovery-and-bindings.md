...
- Cursor: `~/.cursor/plugins/local/<plugin>` and bounded workspace-local plugin roots. A local plugin uses either the Agent Plugins root `plugin.json` or Cursor's `.cursor-plugin/plugin.json`; each manifest may declare `skills` as a string or array of directories, replacing the default `skills/` scan. Without an explicit `skills` field, a plugin with no `skills/` directory may expose one root `SKILL.md`. A local multi-plugin repository may use `.cursor-plugin/marketplace.json`, whose `metadata.pluginRoot` prefixes its plugin sources within that marketplace root.
