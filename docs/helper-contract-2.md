...
If contract 2 is unavailable, incompatible, or malformed, a new dispatcher must stop and Call the Skill tool with its customization type's maintenance skill once; it executes no customization instructions. Maintenance skills use the same installed, approved local-checkout, then approved registry selection order before creation.
