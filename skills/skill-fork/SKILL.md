...
An installed compatible helper requires Node.js only. If unavailable or incompatible, resolve this skill's real path and inspect only its bounded Git ancestors. A checkout is eligible only when it is clean, its `origin` URL normalizes to `https://github.com/samitoyang/skill-customization`, and its root package is named `skill-customization`. Record its canonical root, origin, and commit, then run `npx --yes --package <checkout-root> skill-customization supports 1`. When compatible, reuse that checkout only while its origin, commit, and clean state remain unchanged.

Otherwise explain that `npx` may download `skill-customization@latest` and reuse npm's cache, then obtain permission. After approval, run `npx --yes skill-customization@latest supports 1`; when compatible, use `npx --yes skill-customization@<package_version> <command>` for the remaining commands. Stop with setup guidance if Node.js is missing, if either fallback requires npm and npm is missing, if permission is declined, if output is malformed, if the linked checkout identity or state changes, or if contract 1 is unsupported. A dispatcher delegates here when installed preflight is unavailable or incompatible; it never runs unchecked.
